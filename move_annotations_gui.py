...
class MovieViewer(QMainWindow):
    ...
    def show_help(self):
        ...
        <h2>Mouse action</h2>
        <table>
            <tr><td><b>Left button</b></td><td>Inspect frames (no edit)</td></tr>
            <tr><td><b>Middle button</b></td><td>Add behavior</td></tr>
    ...
    def double_click(self, event: QtGui.QMouseEvent):
        if event.button() == QtCore.Qt.LeftButton:
            x = event.pos().x()
            y = event.pos().y()
            frame = self.point_to_frame(x, y)
